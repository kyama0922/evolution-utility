...
ZipLoader::ZipLoader(EVOLUTION::CORE::FILE::IFileRead& fileloader) : cp_fileloader(&fileloader){ this->TableLoad(); }
ZipLoader::ZipLoader(EVOLUTION::CORE::FILE::IFileRead* fileloader) : cp_fileloader(fileloader){ this->TableLoad(); }

ZipLoader::~ZipLoader(){
	for (auto it = mp_file_table.begin(); it != mp_file_table.end(); it++)
	{
...
s32 ZipLoader::GetData(const char* file_name, TEMPLATE::Array<char>& data){
	...
	u32 hash = Hash::GetInstance()->CRC32((u8*)file_name, strlen(file_name));
	auto zip_folder = this->mp_file_table[hash];
...
void ZipLoader::TableLoad_CentralDirectoryHeader(){
	...
	auto zipfolder = this->mp_file_table[hash];
